...
NLM版本4协议中的过程在语义上与NLM版本3协议中的过程相同。唯一的语义差异是添加了一个可以用来测试服务器响应性的NULL过程。带有\_MSG和\_RES后缀的过程名称表示异步消息；对于这些，void响应意味着不回复。句法更改是过程被重命名以避免与nlm4_stats的值发生名称冲突。因此过程定义如下。
...
Reading Berkshire, RG1 1AX, United Kingdom, 1991. NFS版本2协议和附带协议的不可或缺参考，包括锁管理器和端口映射器。
