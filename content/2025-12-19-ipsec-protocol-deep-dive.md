...
pin: true
tags: [network, security, ipsec, vpn, ike, esp, ah]
---
...
- _规则示例_: "所有从 192.168.1.0/24 到 10.0.0.0/24 的流量，必须使用 ESP 加密。"
- _动作_: Protect (加密), Bypass (直连), Discard (丢弃)。
...
1.  **IKE_SA_INIT (2 packets)**:
    ...
    - _此时，IKE 自身的控制通道已建立，后续所有消息都是加密的。_
