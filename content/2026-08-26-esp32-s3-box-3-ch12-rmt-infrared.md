...
> 上一章：[[2026-08-26-esp32-s3-box-3-ch11-i2s-audio|第十一章：I2S 与音频]]
> **第十二章：RMT 与红外**（当前章）
> 下一章：[[2026-08-26-esp32-s3-box-3-ch13-timers-watchdogs|第十三章：定时器与看门狗]]
...
- **任务被抢占**：你的发码任务翻转 GPIO 的循环里，任何更高优先级任务（比如音频 DMA 半满回调唤起的任务）就绪，你就被换下 CPU。唤醒到恢复的延迟受中断延迟、临界区长度、同优先级时间片多重影响——[[2026-08-26-freertos-deep-dive-series-index|FreeRTOS 深度解析系列]]调度部分拆的正是这些路径，量级在十微秒到毫秒之间波动。
- **软件定时器靠守护任务**：想用软件定时器逐 bit 中断发码也不行——定时器回调排在定时器服务任务的队列后面，回调时刻随系统负载抖动（细节见 [[2026-08-26-freertos-deep-dive-ch15-software-timers-daemon|FreeRTOS（十五）软件定时器]]，12.7 节还会回来算这笔账）。
...
2. 完成通知走 `on_trans_done` 回调——**它在 ISR 环境执行**（头文件注释明言），所以回调里只能做 ISR-safe 的事：`xQueueSendFromISR`、任务通知（`vTaskNotifyGiveFromISR`），把数据转交任务层。这正是第八章 GPIO 中断建立的"ISR 快进快出 + 唤醒任务"模式；任务通知机制本身在 [[2026-08-26-freertos-deep-dive-ch13-task-notifications|FreeRTOS（十三）任务通知]]拆过。
...
软件定时器这条路单独说一句：它的回调由**定时器守护任务**执行，排队长短随系统负载变化，抖动量级远超 NEC 的 ±200us 容差——机制细节见 [[2026-08-26-freertos-deep-dive-ch15-software-timers-daemon|FreeRTOS（十五）软件定时器]]，本章 12.1 的分析在那里有源码级答案。结论：**在 BOX-3 这类多任务系统里，红外收发没有悬念地属于 RMT**；软件方案的价值是帮你理解协议本身。
