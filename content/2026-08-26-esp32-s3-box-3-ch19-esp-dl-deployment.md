...
> 上一章：[[2026-08-26-esp32-s3-box-3-ch18-esp-sr-wakeword|第十八章：ESP-SR 语音唤醒]]
> **第十九章：ESP-DL 模型部署**（当前章）
> 下一章：[[2026-08-26-esp32-s3-box-3-ch20-capstone-voice-remote|第二十章：综合项目]]
...
眼熟吗——`model, data, spiffs` 这一行，与 [[2026-08-26-esp32-s3-box-3-ch4-project-anatomy|第四章]] factory_demo 分区表里那个 8600K 的 `model` 分区（esp-sr 语音模型的住户）是同一个模式：**边缘 AI 设备的 Flash 布局里，"模型分区"是一等公民**，esp-sr 与 esp-dl 在这里殊途同归。
...
为什么要抠这个：19.2/19.4 的官方基准全部跑在"Octal PSRAM + 64KB cache"上，cache 命中主导访存延迟——**同一模型，张量挪进内部 RAM 就有可见的整模型收益**（esp-nn README 的 Person Detection：ESP32-S3 上 54ms，张量用内部 RAM 的版本 47ms）。S3 内部 SRAM 总共 512KB 级，还要养 FreeRTOS 堆、LVGL、音频 DMA，不可能全塞进去。caps 分配的底层机制（`MALLOC_CAP_INTERNAL` vs `MALLOC_CAP_SPIRAM`、各池怎么来的）见 [[2026-08-26-freertos-deep-dive-ch20-idf-heap-and-caps|FreeRTOS 深度解析（二十）]]，此处只取工程结论：
