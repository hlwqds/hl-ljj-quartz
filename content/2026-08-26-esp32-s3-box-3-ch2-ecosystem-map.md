...
> 上一章：[[2026-08-26-esp32-s3-box-3-ch1-quick-start|第一章：三十分钟跑通]]
> **第二章：乐鑫开源生态与资料地图**（当前章）
> 下一章：[[2026-08-26-esp32-s3-box-3-ch3-idf-toolchain|第三章：idf.py 背后——工具链与命令面]]
...
1. **双核 Xtensa LX7 @240MHz**：FreeRTOS SMP 调度的硬件前提（无线协议栈与音频/推理任务可以分核钉住，见 [[2026-08-26-freertos-deep-dive-series-index|FreeRTOS 系列]] Part VI）。
...
- 外设芯片 datasheet（ILI9342C / GT911 或 TT21100 / ES8311 / ES7210 / AHT20）：去各厂商官网取最新版，配合原理图确认信号连接后再精读对应章节（触摸型号两说的定案方法见下文注记与 [[2026-08-26-esp32-s3-box-3-ch9-i2c-sensors|第九章]] 的总线扫描）
...
下一章钻进框架层：`idf.py` 一条命令背后，`install.sh`/`export.sh` 到底装了什么、改了什么环境变量，`set-target esp32s3` 又和经典 esp32 差在哪。构建系统的地基本章已铺好（四种组件来源、分层地图），[[2026-08-26-esp32-s3-box-3-ch3-idf-toolchain|第三章：idf.py 背后]]见。FreeRTOS 系列的同主题章节（[[2026-08-26-freertos-deep-dive-ch3-esp-idf-build-and-bootflow|第三章：ESP-IDF 构建与启动流]]）可交叉对照。
