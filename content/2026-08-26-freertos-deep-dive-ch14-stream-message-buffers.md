...
> [!info] FreeRTOS 深度解析系列 0. [[2026-08-26-freertos-deep-dive-series-index|系列索引]] 14. **第十四章：流缓冲与消息缓冲（单读单写约束）**
...
下一章进入 `timers.c`：软件定时器如何用一个守护任务 + 一条命令队列，把成百上千个定时回调的成本压缩到两个内核对象里——你会看到队列（第十章）与任务通知在这里再次同台，而"定时器命令队列"本身就是一个现成的多写单读队列案例，与本章的汇聚网关模式互为印证：[[2026-08-26-freertos-deep-dive-ch15-software-timers-daemon|第十五章]]。
