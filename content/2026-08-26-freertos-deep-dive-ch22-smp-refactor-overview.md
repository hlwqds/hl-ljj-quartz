...
> [!info] FreeRTOS 深度解析系列 0. [[2026-08-26-freertos-deep-dive-series-index|系列索引]] 22. **第二十二章：IDF FreeRTOS 的 SMP 改造全景**
...
下一章进入双核世界剩下的最后一块硬骨头：**核间同步机制**。`taskYIELD_CORE()` 发出的那个跨核中断究竟怎么送达另一核、自旋锁在 Xtensa 上靠什么原子指令实现、两个核看同一片内存时 cache 一致性由谁保证——第 23 章（[[2026-08-26-freertos-deep-dive-ch23-cross-core-synchronization|第23章]]）把 18 章埋下的 spinlock 伏笔和本章的 IPI 伏笔一并收回。
