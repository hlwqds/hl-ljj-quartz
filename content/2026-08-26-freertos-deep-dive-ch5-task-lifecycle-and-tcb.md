...
> [!info] FreeRTOS 深度解析系列 0. [[2026-08-26-freertos-deep-dive-series-index|系列索引]] 5. **第五章：任务的生与死**
...
3. **`pvPortMalloc` 不是 heap_1~heap_5 里那套**。IDF 用 `components/freertos/heap_idf.c` 把内核的 `pvPortMalloc`/`vPortFree` 直接转接到 IDF heap 组件（`heap_caps_malloc()`/`heap_caps_free()`）。也就是说在 ESP32 上，任务栈和 TCB 与普通 `malloc` 共用同一个多分配器堆，还能吃到 caps 机制——细节留到[[2026-08-26-freertos-deep-dive-ch20-idf-heap-and-caps|第二十章]]。
...
4. **优先级夹紧**：`uxPriority >= configMAX_PRIORITIES` 时截到最大值，然后写入 `pxNewTCB->uxPriority`；开互斥量时同步记录 `uxBasePriority`（优先级继承的基准，[[2026-08-26-freertos-deep-dive-ch11-semaphore-mutex-priority-inheritance|第十一章]]的主角）。
...
还有一层值得知道：ESP-IDF 里另有一棵实验性的 `FreeRTOS-Kernel-SMP/` 树（由 `CONFIG_FREERTOS_SMP` 开启，默认关闭），那是上游 Amazon FreeRTOS SMP 新内核（v11.x 基线）。它的 TCB 又是另一种形状——用位掩码 `uxCoreAffinityMask` 表达亲和（可同时亲和多核），还多出 `xTaskRunState`（记录任务正在哪个核上跑/是否让出中），创建 API 相应变成 `xTaskCreateAffinitySet()`。两条 SMP 路线的对比留给[[2026-08-26-freertos-deep-dive-ch22-smp-refactor-overview|第二十二章]]，本章以下全部聚焦默认树。
...
>
> 跨核 IPI 的硬件机制（`from_cpu` 中断一类）在[[2026-08-26-freertos-deep-dive-ch23-cross-core-synchronization|第二十三章]]展开。
...
跑起来（环境搭建见[[2026-08-26-freertos-deep-dive-ch1-from-bare-metal-to-rtos|第一章]]）：
...
在断点处能看到一个"半成品"任务：`pcTaskName` 已填、`pxTopOfStack` 已指向伪栈帧、而 `xStateListItem` 还没有 `pxContainer`——再单步越过 `prvAddNewTaskToReadyList()`，它就挂进了 `pxReadyTasksLists[4]`。静态创建的话，把断点换成 `xTaskCreateStaticPinnedToCore` 能看到同一个 TCB 从你的静态缓冲区里"长"出来。更多调试姿势见[[2026-08-26-freertos-deep-dive-ch24-debugging-tracing-pitfalls|第二十四章]]。
...
任务已经"活"了，也挂进了 `pxReadyTasksLists[]`——但这条链表凭什么叫"就绪"？调度器每一步怎么从一堆链表里挑出下一个上 CPU 的任务、双核又怎么各自为政还互不踩脚？下一章钻进调度器的选路算法：[[2026-08-26-freertos-deep-dive-ch6-scheduler-ready-lists|第六章：调度器核心——就绪链表与最高优先级任务选择]]。
