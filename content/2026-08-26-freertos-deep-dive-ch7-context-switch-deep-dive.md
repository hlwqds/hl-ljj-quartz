...
> [!info] FreeRTOS 深度解析系列 0. [[2026-08-26-freertos-deep-dive-series-index|系列索引]] 7. **第七章：上下文切换：从 portYIELD() 到汇编的每一步**
...
此后 ISR 的 C 代码全部跑在**独立的每核中断栈**上（`port.c` 定义 `port_IntStack[portNUM_PROCESSORS][configISR_STACK_SIZE]`），任务栈只需容纳一个中断帧的空间——这就是 IDF 任务栈可以给得比想象小的原因之一（栈预算的完整账本见[[2026-08-26-freertos-deep-dive-ch21-stack-and-memory-layout|第21章]]）。若已是嵌套中断，则继续用当前系统栈，只加计数。
...
在第一章工程基础上，把 `main/main.c` 换成同时制造三条路径的最小程序：
...
`coop_a`/`coop_b` 同优先级且钉在 Core 0，`taskYIELD()` 会在两者间严格交替——路径①的完美样本；`sleeper` 优先级更高但每 200ms 阻塞一次，每次阻塞是路径②、每次被 tick 唤醒抢走 CPU 是路径③。注意 `coop_*` 若不钉核，SMP 调度器可能把它们分到两个核上各自空转，`taskYIELD()` 的交替现象反而看不见——钉核正是为了让实验确定可复现（亲和对调度的影响见[[2026-08-26-freertos-deep-dive-ch22-smp-refactor-overview|第22章]]）。
...
QEMU 里看不到的只有真实中断延迟与时序抖动（例如 CCOMPARE 补拍循环的触发频率），逻辑层面两边无差别。更多调试技巧（trace、任务快照、常见翻车案例）在[[2026-08-26-freertos-deep-dive-ch24-debugging-tracing-pitfalls|第24章]]系统展开。
