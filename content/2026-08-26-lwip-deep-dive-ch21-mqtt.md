...
> [!info] lwIP 深度解析系列 0. [[2026-08-26-lwip-deep-dive-series-index|系列索引]] 21. **第二十一章：MQTT：esp-mqtt 架构与弱网行为**
...
第三种的实战意义：**RST 不是死亡证据而是"对端确实不存在"的证据**，它与 FIN 都比无声冻结友好得多——这与 [[2026-08-26-lwip-deep-dive-ch11-tcp-state-machine|第十一章]] 的半开连接讨论严丝合缝：半开的可怕恰恰在于两端都没错，只是对面没了。
...
> [!tip] ch22 的伏笔
> 本章构建日志里 mbedtls 的目标文件赫然在列——那是 `MQTT_TRANSPORT_SSL=y` 经由 tcp_transport→esp-tls 拖进链接的，即使我们只用明文 `mqtt://`。下一章 [[2026-08-26-lwip-deep-dive-ch22-tls-esp-tls-mbedtls|第二十二章]] 就拆这条"mqtts 到底多贵"的链路。
...
工程位于 `practice/lwip-ch21-mqtt/`，沿 [[2026-08-26-lwip-deep-dive-ch3-qemu-network-lab|第三章]] openeth bring-up 模板，main 组件 manifest 增加 `espressif/mqtt: "^1.0.0"`。关键事实：SLIRP 下 guest 访问 `10.0.2.2:<port>` 落到宿主机 loopback 同端口，因此固件直连 `mqtt://10.0.2.2:1883` 即达宿主机 mosquitto，**全程不需要 hostfwd**。
...
下一章进入安全层：`mqtts://` 背后 tcp_transport→esp-tls→mbedtls 的整条链路——握手要几个 RTT、多少堆、多少任务切换，以及证书验证失败时那一串 error code 分别是谁吐的。见 [[2026-08-26-lwip-deep-dive-ch22-tls-esp-tls-mbedtls|第二十二章]]。
