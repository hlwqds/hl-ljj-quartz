...
> [!info] lwIP 深度解析系列 0. [[2026-08-26-lwip-deep-dive-series-index|系列索引]] 23. **第二十三章：调试工具箱：LWIP_DEBUG、统计与抓包**
...
源码参照：ESP-IDF **v6.0.2**（`~/esp/esp-idf`），捆绑 lwIP **2.2.0-dev**（`components/lwip/lwip/src`）。所有 Kconfig 名、宏名、函数行为均以该仓库源码与本章实测为准。本章是 [[2026-08-26-freertos-deep-dive-ch24-debugging-tracing-pitfalls|FreeRTOS 系列第二十四章]] 的网络篇姊妹章——那边教你怎么停住整个世界看任务，这边教你协议栈坏掉时怎么一层层剥开看字节。
...
- **错误码别望文生义**：lwIP 的 errno 映射有自己的方言——`accept/connect` 拿到 `113(EHOSTDOWN)` 多半是"你的 SYN 被吞了"，`close` 返回 `128` 是跨任务解阻塞哨兵（[[2026-08-26-lwip-deep-dive-ch16-socket-netconn-vfs|第十六章]]）。对照表收在 23.7。
...
表的意义在于**前后差**。[[2026-08-26-lwip-deep-dive-ch9-ip4-icmp|第九章]]给过 `snap_take/snap_diff` 范式，核心十行（本章实验工程直接内置）：
...
陷阱两条（[[2026-08-26-lwip-deep-dive-ch22-tls-esp-tls-mbedtls|第二十二章]]用血泪修正过的口径）：min-ever 是**多区域 sum-of-minima**，跨越阶段直接相减会失真，正确姿势是"阶段边界差 + 周期采样窗最小值"双指标并用；free 正常回涨不代表没事，要看 largest 有没有跟着涨（碎片化三联征：free 回涨 / largest 不动 / min-ever 单调下降，见 [[2026-08-26-lwip-deep-dive-ch5-memory-management|第五章]]）。
...
**姿势 1，暂停起步抓分配路径**（配合 [[2026-08-26-freertos-deep-dive-ch24-debugging-tracing-pitfalls|FT 系列第廿四章]]的双终端直觉）：
...
`tools/ch23_tcpalloc.gdb` 内容：`target remote :1234` → `break tcp_alloc` → `continue` → `bt`。首次命中的真实 backtrace——一条直线证明协议栈是单线程邮箱模型（[[2026-08-26-lwip-deep-dive-ch13-tcpip-thread-mailbox|第十三章]]的活化石）：
...
工程：`practice/lwip-ch23-debugging-toolbox/`（构建/运行命令见其 README，或 23.9 的最小复现集）。目标悬案来自 [[2026-08-26-lwip-deep-dive-ch22-tls-esp-tls-mbedtls|第二十二章]] 的留档记录："**guest 内 loopback 明文服务仅第一条连接可用，后续 connect 全 RST**"。
...
下一章进入本系列收官：《性能与调优陷阱》。调试章备好了"哪里不对劲"，性能章回答"哪里可以更快"——吞吐预算怎么拆、每层抽成多少、哪些优化是真优化哪些是安慰剂，以及把整套方法论交给下一个接手你设备的人之前，如何写成不会被忽略的文档。见 [[2026-08-26-lwip-deep-dive-ch24-performance-tuning-pitfalls|第二十四章]]。
