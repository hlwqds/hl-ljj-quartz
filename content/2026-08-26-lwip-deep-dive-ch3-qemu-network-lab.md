...
| 3   | 退出码误判                             | `timeout ... ; echo $?` 得 124 而应用明明活着                                           | 124 = timeout 正常截停 QEMU；应用主动崩溃复位后因 `-no-reboot` 则以 0/-1 干净退出；如果像本章首次那样接了管道（`tee                                                                                                                                     | grep`），杀进程时管道成员可能收 SIGTERM 报 143——看 log 文件本身最稳 |
