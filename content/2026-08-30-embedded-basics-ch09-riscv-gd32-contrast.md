...
tags: [embedded, riscv, gd32, stm32, cortex-m, architecture]
---

> [!info] 嵌入式硬件基础系列 0. [[2026-08-30-embedded-basics-series-index|系列索引]] · 8. [[2026-08-30-embedded-basics-ch08-timer-systick|上一章：定时器]] · 9. **第九章：RISC-V 侧起点** · 10. [[2026-08-30-embedded-basics-ch10-i2c-spi-theory|下一章：I2C/SPI 协议理论]]（并行写作中）

# 嵌入式硬件基础（九）：RISC-V 侧起点——GD32VF103 与双架构对照

[[2026-08-30-embedded-basics-ch01-roadmap-and-boards|第一章]] 说过本系列的学习法：GD32VF103 是 STM32F103 的"寄存器级双胞胎"——兆易创新把 F103 的身体（外设、总线、内存映射）几乎原样保留，心脏换成了芯来科技的 Bumblebee 内核（RV32IMAC，108MHz）。前面八章我们把 Cortex-M 侧从寄存器组、向量表、启动文件一路打到定时器；现在换阵营，同一个问题集在 RISC-V 上重新问一遍——**答案相同的地方是"外设知识"，答案不同的地方就是"架构知识"**。
...
[[2026-08-30-embedded-basics-ch03-arm-cortex-m-anatomy|第三章]] 里 Cortex-M 的控制状态是这么藏的：
...
[[2026-08-30-embedded-basics-ch04-baremetal-boot|第四章]] 的 Cortex-M 启动三件套（向量表+Reset_Handler+ld）在 RISC-V 侧逐件有镜像，但接线不同。以下全部对照 nuclei-sdk 官方文件：`SoC/gd32vf103/Common/Source/GCC/startup_gd32vf103.S` 与 `Board/*/Source/GCC/gcc_gd32vf103_flashxip.ld`。
...
下一章 [[2026-08-30-embedded-basics-ch10-i2c-spi-theory|嵌入式硬件基础（十）：I2C/SPI 协议理论]] 回到**架构无关**的领域：芯片之间的总线协议。时序图、地址与 ACK、CPOL/CPHA——这些知识在两块板上长着同一张脸，因为它们住在比 ISA 更低的层：电气与协议层。读完 ch10，你离"从比特到引脚"的全栈就只差调试体系（[[2026-08-30-embedded-basics-ch11-debug-swd-jtag|第十一章]]）和 RTOS 会师（[[2026-08-30-embedded-basics-ch12-freertos-port-contrast|第十二章]]，PendSV vs machine timer 的终局对照）了。
...
- 本系列 [[2026-08-30-embedded-basics-ch03-arm-cortex-m-anatomy|第三章]]（异常 8 寄存器压栈实测、EXC_RETURN、MSP/PSP）、[[2026-08-30-embedded-basics-ch04-baremetal-boot|第四章]]（Reset_Handler/链接脚本/两段式加载）、[[2026-08-30-embedded-basics-ch07-interrupts-nvic|第七章]]（NVIC 寄存器族）、[[2026-08-30-embedded-basics-ch08-timer-systick|第八章]]（SysTick 与 machine timer 流派）
...
- FreeRTOS 系列衔接：[[2026-08-26-freertos-deep-dive-ch16-portmacro-port-contract|portmacro 契约]]、[[2026-08-26-freertos-deep-dive-ch17-xtensa-port-internals|Xtensa 端口深挖]]（第十二章三架构对照的前置）
