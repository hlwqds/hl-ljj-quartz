...
tags: [STM32, JTAG, Debug, Lab]
---

# JTAG 调试口深潜：五根线、一个状态机和它的本职工作（含实测）

> **状态声明**：本章证据最厚的一篇——本系列**从序章救板到 ch22 的全部烧录与调试，
...
两个 `JTAG tap` 行 = 链上有**两个 TAP**被逐个识别。第二行那个 `0x06419041` 在砖机
...
[[2026-08-30-f429-lab-ch00a-swd-debug-port|ch00a 第 2.2 节]]）。所以对调试用户而言
...
2×5 防呆盒座一次接通全部 10 触点（含 JTAG 全部五信号）。挑战者调试座与 DAP 的缺口
对缺口，物理唯一。
...
| `make serial` | 串口监听（115200 8N1） | 救板夜看到的第一坨乱码与修复      |
| `make probe`  | 体检                   | 砖机之夜的身份鉴定                |
...
- [[2026-08-30-f429-lab-ch00a-swd-debug-port|ch00a：SWD 调试口深潜]]——姊妹篇：两根线与三层架构详解
- [[2026-08-30-stm32f429-brick-rescue-debug-story|序章：救板实录]]——JTAG TAP 身份鉴定的实战现场
