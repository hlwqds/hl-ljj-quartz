...
> [[2026-08-30-stm32f429-brick-rescue-debug-story|序章：救板实录]]。
...
压到最低，原因见[[2026-08-26-freertos-deep-dive-ch18-critical-sections-spinlocks|FreeRTOS（十八）：临界区契约]]）。
...
被业务中断拦腰打断，这是[[2026-08-26-freertos-deep-dive-ch18-critical-sections-spinlocks|FreeRTOS（十八）]]
...
[[2026-08-26-freertos-deep-dive-ch16-portmacro-port-contract|FreeRTOS（十六）：portmacro 移植契约]]
...
| #   | 项                                                         | 核对方法                      |
| --- | ---------------------------------------------------------- | ----------------------------- |
| 1   | CALIB 实测值（TENMS/SKEW/NOREF 位段）                      | `mdw 0xE000E01C`，对照 RM0090 |
| 2   | LED 引脚号（J73 相关）                                     | 丝印/原理图，代码宏占位待回填 |
| 3   | CTRL bit16 是否呈现「随节拍抖动 + 重复读后归零」的读清语义 | 连续 `mdw` 两次对比           |
| 4   | 节拍周期 1.000s 的实际累积漂移（晶振偏差）                 | 串口时间戳跑 10 分钟对表      |
| 5   | FreeRTOS 固件下 SHPR3 高字节读回 `0xF0F0`                  | `mdw 0xE000ED20`              |

下一章：[[2026-08-30-f429-lab-ch02-button-polling-to-interrupt|按键三态：轮询、中断与消抖]]——
