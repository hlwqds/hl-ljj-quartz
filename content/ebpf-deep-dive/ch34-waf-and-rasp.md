...
> 29. [[ch21-debugging-and-verifier|第二二十一|第二十一章：调试实战与验证器 (Verifier) 诊断]]
...
A：XDP 层无法解密 HTTPS 流量。但有两个互补方案：1) 使用 TC 层在 TLS 握手后、加密前拦截明文（[[ch7-5-uprobes-dynamic-tracing|uprobe on SSL_write]]）；2) 使用 XDP 做基于 IP/Port/包大小的行为检测（如检测心包模式、异常连接频率）。
...
A：关键措施：1) 限制 `CAP_BPF` 权限，仅允许特权用户加载 BPF 程序；2) 启用 BPF 签名验证（[[ch31-signed-objects-and-security|第三十一章]]）；3) 使用 LSM BPF 保护 eBPF 系统调用本身；4) 定期审计已加载的 BPF 程序列表。
