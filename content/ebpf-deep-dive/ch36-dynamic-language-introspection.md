...
A：Python 3 的字符串内部是 PyUnicodeObject，支持多种编码（UTF-8、Latin-1、UCS-4 等）。通过 `ob_type->tp_name` 确认是 `str` 类型后，根据 `PyUnicode_KIND` 判断编码类型。UTF-8 和 Latin-1 可以直接读取；UCS-4 需要读取 `wchar_t` 数组。[[ch7-5-uprobes-dynamic-tracing|第七.五章：uprobe 用户态动态追踪原理]] 详细介绍了 Unicode 处理。
