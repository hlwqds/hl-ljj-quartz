...
A：不能直接解密。QUIC 使用 TLS 1.3 加密所有载荷（包括头部的大部分字段）。eBPF 能看到的是：1) 外层 UDP/IP 头（源/目标 IP 和端口）；2) QUIC 的连接 ID（用于路由，明文）；3) Initial 包中的 TLS ClientHello（明文，包含 SNI）。如果需要解密 QUIC 流量进行深度检测，需要使用 eBPF 配合 SSLKEYLOGFILE 或 uprobe 拦截 OpenSSL/BoringSSL 的密钥导出函数（见[[ch26-network-security-high-level|第二十六章]]）。
