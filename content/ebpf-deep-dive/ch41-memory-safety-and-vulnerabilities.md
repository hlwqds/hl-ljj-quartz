...
A：监控方案：1) 使用 Tetragon 或 Falco 监控 BPF 系统调用（`bpf` syscall）的调用频率和来源；2) 定期审计已加载的 BPF 程序列表（`bpftool prog list`）；3) 监控 BPF Map 的访问模式，检测异常（如 Map 大小突然增长）；4) 启用 eBPF 自愈监控（见[[ch24-meta-monitoring-and-self-healing|第二十四章]]）。
