...
| self-claim 不发生 / `sec_cert` 分区空                      | `CONFIG_ESP_RMAKER_SELF_CLAIM=y` 或 `sec_cert`/`fctry` 分区被裁掉；或设备访问不到 claiming 服务                                             | 保留 [[ch4-project-anatomy\|第四章]]那张分区表的 `sec_cert`/`fctry` 行与配置项，确认网络可达后重新上电触发                                                           |
