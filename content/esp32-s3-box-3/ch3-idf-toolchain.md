...
> 上一章：[[ch2-ecosystem-map|第二章：乐鑫开源生态与资料地图]]
> **第三章：idf.py 背后——工具链与命令面**（当前章）
> 下一章：[[ch4-project-anatomy|第四章：工程解剖与组件体系]]
...
- **安装命令怎么敲**：见 [[ch1-from-bare-metal-to-rtos|FreeRTOS 系列第一章]] 1.4 节，本章不重复，只讲命令背后的机制；
- **CMake 构建体系内部**（project.cmake 如何组织组件、链接脚本、启动流程）：属于 [[ch3-esp-idf-build-and-bootflow|FreeRTOS 系列第三章]] 的领地，本章只到"idf.py 把工作交给 cmake/ninja"这条边界为止。
...
**事实一：v6 的 install.sh 不装系统依赖。** 脚本里没有任何 `apt-get`/`dnf` 调用——"检测/安装系统依赖"是老版本（v3/v4 时代）的行为，v6 只**检测 Python**（`detect_python.sh` + `python_version_checker.py`）。缺 `git`、`libusb` 之类系统包时，它会直接在下载/使用环节报错，而不是替你装。这也是 [[ch1-from-bare-metal-to-rtos|FreeRTOS 第一章]] 要单独 `dnf install` QEMU 运行库的原因。
...
本机此刻只登记了 `esp32s3` 一个 target——正好印证系列索引的约定（[[esp32-s3-box-3|系列索引]]：与 FreeRTOS 系列共用一套 IDF）。之后想补 esp32，重跑 `./install.sh esp32` 即可，target 列表会**累积合并**；而且由于两条工具链大量共享（见 3.5），这次补装几乎是零下载。
...
最后一句要点：idf.py **不是必需品**——脚本头部注释明说"You don't have to use idf.py, you can use cmake directly"。它是 cmake/ninja/esptool 的便捷编排层；剥掉它之后构建系统长什么样，是 [[ch3-esp-idf-build-and-bootflow|FreeRTOS 系列第三章]] 的主题。
