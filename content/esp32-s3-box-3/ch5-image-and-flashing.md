...
> 上一章：[[ch4-project-anatomy|第四章：工程解剖与组件体系]]
> **第五章：编译产物与烧录链路**（当前章）
> 下一章：[[ch6-boot-to-app-main|第六章：从上电到 app_main]]
...
（bootloader 为什么是独立子工程、`sdkconfig.h` 怎么从 sdkconfig 生成，分别在 [[ch3-esp-idf-build-and-bootflow|FreeRTOS（三）]]与[[ch4-project-anatomy|第四章]]4.5 节讲过，不重复。）
...
对照 BOX-3 出厂固件用的自定义表（[[ch4-project-anatomy|第四章]]4.2 节已逐行拆解，此处只对比骨架）：
...
一行读出四件事：函数名、地址 `0x4200c8e8`、大小 `0x5` 字节（hello world 确实只有 5 字节）、来自 `main` 组件的 `hello-s3.c.obj`。注意 `0x4200xxxx` 不是 Flash 内偏移，而是 **IROM 虚拟地址窗口**里的链接地址——虚拟地址与 Flash 物理偏移的换算由镜像段表与 MMU 映射承担，机制的归属见 [[ch21-stack-and-memory-layout|FreeRTOS（二十一）内存布局]]。
...
想只看分区不动数据，用 `parttool.py`（住在 `components/partition_table/`，export 已把它注入 PATH——[[ch3-idf-toolchain|第三章]]3.3 节核实过）：
