...
> 上一章：[[ch5-image-and-flashing|第五章：编译产物与烧录链路]]
> **第六章：从上电到 app_main**（当前章）
> 下一章：[[ch7-system-services|第七章：系统服务层]]
...
老教程说入口是 `components/bootloader_support/src/bootloader_start.c`——v6 里它移到了 **`components/bootloader/subproject/main/bootloader_start.c`**（bootloader 是独立子工程，[[ch3-esp-idf-build-and-bootflow|FreeRTOS（三）]]3.1 节讲过构建视角），且全芯片共用一份。目标相关初始化拆去了 `src/esp32s3/bootloader_esp32s3.c`。入口函数与注释值得整段读：
...
这个 `entry_addr` 就是应用镜像的入口函数——`call_start_cpu0`（下一节）。XIP 段（代码/只读数据）**不搬运**，靠刚建好的映射就地执行；只有 data/bss 落 RAM 的段被实际拷贝。MMU 映射的构建细节归 [[ch3-esp-idf-build-and-bootflow|FreeRTOS（三）]]3.3 节管，此处不重复。
...
| 维度       | 本篇（工程视角）                        | [[ch3-esp-idf-build-and-bootflow\|FreeRTOS（三）]]（构建/内核视角） |
| ---------- | --------------------------------------- | ------------------------------------------------------------------- |
