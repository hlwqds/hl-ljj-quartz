...
tags: [f429-lab, STM32, GPIO, Interrupt]
---
...
- **优先级契约**：本工程 `configMAX_SYSCALL_INTERRUPT_PRIORITY = 5<<4`，意味着优先级数值
  ...
  与[[ch18-critical-sections-spinlocks|FreeRTOS（十八）：临界区契约]]
  理论篇互为表里。
...
上一章：[[ch01-systick-heartbeat|SysTick：内核的心跳]]（g_ms 的来历）；
下一章：[[ch03-timer-pwm-buzzer|定时器与 PWM：让蜂鸣器唱歌]]。
