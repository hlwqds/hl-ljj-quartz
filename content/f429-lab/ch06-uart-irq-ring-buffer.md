...
tags: [f429-lab, STM32, UART, Interrupt]
---
...
3. **发布顺序：先写数据槽，再推进索引**。`volatile` 挡住编译器重排；CM4 单核下程序序
   ...
   详见[[ch18-critical-sections-spinlocks|FreeRTOS（十八）：临界区契约]]。
...
- [[ch13-tcpip-thread-mailbox|lwIP（十三）：tcpip 线程信箱]]是同一
...
上一章：[[ch05-bus-matrix-ccm|总线矩阵：CCM 为什么是「近端内存」]]；
下一章：[[ch07-spi-flash-w25q64|SPI：点亮板载 8MB Flash]]——
