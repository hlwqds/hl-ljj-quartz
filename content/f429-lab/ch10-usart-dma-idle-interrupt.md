...
- 收发路径从此分层：**数据面走 DMA，控制面走中断**——正是
  [[ch17-ethernetif-porting-guide|lwIP（十七）：ethernetif 移植]]
...
- DMA 生产者 + 任务消费者 = ch06 SPSC 环（single-producer single-consumer 单生产者单消费者
  ...
  [[ch14-stream-message-buffers|FreeRTOS（十四）：Stream 与 Message Buffer]]；
...
上一章：[[ch09-rpi-piscope-logic-analyzer|波形之眼：树莓派低成本逻辑分析仪]]；
下一章：[[ch11-adc-dma-double-buffer|ADC+DMA 双缓冲：无 CPU 的采样流]]——
把本章的 HT/TC 两个位变成主角。系列总目录见[[f429-lab|F429 裸机实验室索引]]。
