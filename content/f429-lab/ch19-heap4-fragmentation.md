...
tags: [f429-lab, STM32, FreeRTOS, Heap]
---
...
> [[ch14-stack-units-word-vs-byte|ch14：栈的单位]]严格一致，观察工具承接
> [[ch16-stack-high-water-mark|ch16]]与[[ch17-pendsv-single-step|ch17]]。
...
- 对照：[[ch19-heap-allocators-comparison|FreeRTOS（十九）]]在 ESP32 侧
  讲过四种分配器的理论（heap_2 无合并的 A/C 演示、heap_4 的 first-fit+合并），
  [[ch20-idf-heap-and-caps|FreeRTOS（二十）]]是 IDF 的多堆 TLSF——本章在
  64KB 的单堆上把同一套数字跑成实拍。
...
[[ch19-heap-allocators-comparison|FreeRTOS（十九）]]19.7 有整节论证，此处只记
...
下一章：[[ch20-ethernet-emac-phy|以太网上电：EMAC 与 PHY]]——
