...
tags: [f429-lab, STM32, LwIP, Ethernet]
---
...
[[ch17-ethernetif-porting-guide|lwIP（十七）]]17.4 的 A/B 方案表）：
...
[[ch08-instrument-roles-guide|RPi 仪器手册]]）：
...
- **RPi → F429**：`ping 10.42.0.10`——lwIP 收到 ICMP echo request 自动回（`LWIP_ICMP=1` 即可，
  无需一行应用代码；全路径见[[ch9-ip4-icmp|lwIP（九）：IP 与 ICMP]]）。
...
[[ch14-sys-arch-freertos-adapter|lwIP（十四）：sys_arch 缝合层]]的裸机作业
...
- **lwIP 主系列闭环**：主系列 ch17 讲「移植契约一页纸」，本章把契约兑现到真 MAC；ch18 的 wlanif
  ...
  `f429-boot.pcap` 可直接套用[[extra-pcapx-pluggable-capture|pcapx 附加篇]]。
...
- **FreeRTOS 侧**：tcpip_thread 的栈深/优先级是
  [[ch19-isr-and-priority-design|lwIP（十九）]]的作业——实测后回填。
...
上一章：[[ch20-ethernet-emac-phy|以太网上电：EMAC 与 PHY]]——硬件邮箱已上架；
下一章：[[ch22-esp32-vs-f429-throughput|全家对照：ESP32 vs F429]]。系列总目录见
