...
> [!info] FreeRTOS 深度解析系列 0. [[freertos-deep-dive|系列索引]] 13. **第十三章：任务通知：最轻量的 IPC**
...
既然每个任务生来就有一个 TCB（[[ch5-task-lifecycle-and-tcb|第五章]] 解剖过），最省的做法不是再造对象，而是**直接在 TCB 里预留一对字段**当邮箱。`tasks.c` 的 TCB 定义里就是这样一段（由 `configUSE_TASK_NOTIFICATIONS` 控制）：
...
2. **临界区内竟然可以 yield**。源码注释原话：所有端口都支持在临界区内 yield——有的立即切换，有的等临界区退出再切。Xtensa 属于后者：`portYIELD_WITHIN_API()` 的定义是 `esp_crosscore_int_send_yield(xPortGetCoreID())`——**给自己发一个软件中断**，此刻中断被关着，它会在 `taskEXIT_CRITICAL()` 恢复使能的瞬间触发，切换在那里完成（机制细节归 [[ch7-context-switch-deep-dive|第七章]] 与 [[ch17-xtensa-port-internals|第十七章]]）。
...
下一章离开"值"的世界，进入"流"的世界：`stream_buffer.c` 的流缓冲与消息缓冲——FreeRTOS 最年轻的 IPC 成员，用"单读单写"的约束换来近乎无锁的零拷贝式传递，是通知（事件）与队列（数据）之间的第三条路。见 [[ch14-stream-message-buffers|第十四章]]。
