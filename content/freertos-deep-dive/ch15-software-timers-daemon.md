...
> [!info] FreeRTOS 深度解析系列 0. [[freertos-deep-dive|系列索引]] 15. **第十五章：软件定时器：守护任务与命令队列**
...
下一章离开内核对象，进入端口层：`portmacro.h` 里每一个宏（`portYIELD`、`portENTER_CRITICAL`、`portTICK_TYPE`……）背后都站着一条具体的硬件事实。它是内核与 CPU 之间的契约文书，也是读懂后续上下文切换汇编与临界区实现（第 17、18 章）的入场券——见 [[ch16-portmacro-port-contract|第十六章]]。
