...
> [!info] lwIP 深度解析系列 0. [[lwip-deep-dive|系列索引]] 10. **第十章：UDP：PCB 匹配与三层 API**
...
下一章进入 TCP 的领土。有了本章的 PCB 匹配作参照系，会发现 tcp_pcb 一出场就是完全不同的物种：七种状态组成的状态机、三次握手里的 SYN/ACK 序号博弈、以及 `tcp_process()` 那张覆盖一切的迁移大表——[[ch11-tcp-state-machine|第十一章《TCP：状态机与三次握手》]]。
