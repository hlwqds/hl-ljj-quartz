...
> [!info] lwIP 深度解析系列 0. [[lwip-deep-dive|系列索引]] 11. **第十一章：TCP 状态机：tcp_input 与连接的一生**
...
PCB 的"容器"比 UDP（[[ch10-udp-pcb-layers|第十章]]）复杂一档——四张链表加一条不变式，定义于 `src/include/lwip/priv/tcp_priv.h`：
...
而 `struct tcp_pcb`（active 家族）则多出约 40 个字段：远端四元组补全（`remote_port`）、重传账本（`rtime/rto/nrtx`）、RTT 估计（`sa/sv`）、拥塞窗口（`cwnd/ssthresh`）、三个段队列（`unsent/unacked/ooseq`）、keepalive 参数（`keep_idle/keep_intvl/keep_cnt`）……内核名场面字段在 [[ch12-tcp-reliability-flow-control|第十二章]]逐一出场。
...
应用调 `close()` 并不总是优雅三条腿。`tcp_close_shutdown(pcb, rst_on_unacked_data)` 开头有个短路分支：ESTABLISHED/CLOSE_WAIT 态下，若有未送达应用的数据（`refused_data != NULL` 或接收窗口未复原），直接**先发 RST 再立刻销毁**——用于通知对方"数据没消化完，别假装体面"。这也是 [[ch16-socket-netconn-vfs|socket 层]] close() 与 raw 层 tcp_close() 行为差异的来源之一。
...
下一章走进 TCP 的"内容世界"：重传如何记账、RTT 如何估摸、cwnd 如何爬坡，以及接收窗口如何反向塑造发送方的心跳——可靠性追求的细节数不胜数，见 [[ch12-tcp-reliability-flow-control|第十二章：TCP 可靠性与流量控制]]。
