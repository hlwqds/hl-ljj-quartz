...
| sys_thread_new 栈单位 | `stacksize / sizeof(StackType_t)`（lwIP 字节数→FreeRTOS 字，或有字数开关）   | 字节数直传（IDF 内核 `portSTACK_TYPE=uint8_t`，参见 [[ch16-portmacro-port-contract\|FreeRTOS 十六章]] 的类型契约） |
