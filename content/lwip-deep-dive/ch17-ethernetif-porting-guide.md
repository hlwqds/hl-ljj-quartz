...
> [!info] lwIP 深度解析系列 0. [[lwip-deep-dive|系列索引]] 17. **第十七章：ethernetif 移植指南：把 lwIP 搬上任意 MCU**
...
1. **pbuf 抽象**（[[ch4-pbuf-anatomy|第四章]]）：驱动永远只面对 pbuf 或裸 buffer，不必理解 IP 分片重组。
...
这就是第十九章将展开的「ISR 极简主义」现场版：贴标签（读状态）、叫人（Task Notify，参见 [[ch13-task-notifications|FreeRTOS ch13]]）、关门（清标志）。没有任何拷贝、没有分配、没有 pbuf——**因为 ISR 里那些事每一样都可能成为最坏情况延迟的引爆点**。值得咀嚼的是 BUSY 分支：它说明缺缓冲的责任边界在硬件，INT_BUSY 就是硬件开的"丢条数从这里开始计"的发票。
