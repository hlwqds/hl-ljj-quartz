...
> [!info] lwIP 深度解析系列 0. [[lwip-deep-dive|系列索引]] 6. **第六章：零拷贝：tcp_write 的 copy 语义与数据通路**
...
下一章顺着数据通路的地图走向下一站基础设施：那条把它们全部串起来的抽象层 `netif`——注册、路由选择、linkoutput 约定、以及 6.2 图里被我们一笔带过的 ESP-IDF 组合牌（esp_netif 与 lwIP netif 的组合关系）。见 [[ch7-netif-abstraction|第七章《netif 抽象层》]]。
