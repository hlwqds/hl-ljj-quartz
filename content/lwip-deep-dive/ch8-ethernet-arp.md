...
> [!info] lwIP 深度解析系列 0. [[lwip-deep-dive|系列索引]] 8. **第八章：以太网与 ARP：从帧到 IP 的第一跳**
...
[[ch4-pbuf-anatomy|第四章]]量过 `pbuf layer` 各档的头偏移：LINK=14、IP=54、TRANSPORT=74（IPv6 开启时编译值）。那串数字的本体就是本章的两行代码——`ethernet_input()` 对每个可识别类型做同一次"层权交接"：
...
帧的世界到此收束：本章结束时，payload 指针已经稳稳落在 IP 头上。下一章顺着这个指针继续剥——[[ch9-ip4-icmp|第九章]]深入 `ip4_input()` 的校验、分片重组与转发判定，外加 ICMP：ping 到底经历了什么、回显报文在哪一行拐弯，让我们把"网关能 ping 通"这句话拆成字节级别的真相。
