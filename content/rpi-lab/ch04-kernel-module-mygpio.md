...
tags: [rpi-lab, RPi, Linux, Driver]
---

# 内核模块入门：hello 与 my_gpio 字符设备

> **状态声明**：本章为「先成文、后实跑」的实验设计，**未在真机执行**；以
> [[ch01-bookworm-surgery-baseline|ch01]] 升级后的 Bookworm（内核 ≥ 6.6）
...
- **insmod 之于烧录**，就是给跑着的发动机换火花塞之于整台换发动机——合法且快，但手一抖
  整机爆缸；[[ch05-request-irq-vs-nvic|ch05（request_irq）]]把威胁模型放大到中断上下文。
- my_gpio 故意硬编码 `0xFE200000`——F429 世界里「代码里写死地址」天经地义，Linux 世界里
  这是被设备树取代的旧习俗，[[ch06-device-tree-overlay-spi|ch06]] 专门清算它。
