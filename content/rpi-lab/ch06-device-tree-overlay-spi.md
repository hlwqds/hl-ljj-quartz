...
tags: [rpi-lab, RPi, Linux, DeviceTree]
---

# 设备树：嵌入式的 BIOS

> **状态声明**：本章为「先成文、后实跑」的实验设计，**未在真机执行**；以
> [[ch01-bookworm-surgery-baseline|ch01]] 升级后的 Bookworm（内核 ≥ 6.6）为
...
| 换板重编译               | 移植 = 重查一遍晶振/引脚重编译（序章晶振 25M 陷阱）  | 同一驱动 + 不同 DTB/overlay        |
...
下一章 [[ch07-read-syscall-fullstack-trace|ch07]] 拿本章的 spiloop 当靶子，
