---
title: 仪器手册：低成本逻辑分析仪与网络之眼
date: 2026-08-30 04:05:00
description: RPi4B 六种仪器角色速查——piscope 低成本逻辑分析仪、i2cdetect 主机扫描、tcpdump/iperf3 网络对端、hostapd 受控 AP、usbmon USB 之眼：每节用途/最小命令/预期输出/对接章节/已知坑
tags: [rpi-lab, RPi, Linux, Instrument]
---

# 仪器手册：低成本逻辑分析仪与网络之眼

> **状态声明**：本章为速查手册体裁，**未在真机执行**；以
> [[ch01-bookworm-surgery-baseline|ch01]] 升级后的 Bookworm 为前提。所有
...
| 词                     | 一句话解释                                                       |
| ---------------------- | ---------------------------------------------------------------- |
| pigpiod / piscope      | GPIO 采样守护进程 + 远程波形 GUI：两者拼成低成本逻辑分析仪       |
...
| #   | 角色      | 一句话用途               | 主命令                  | 对接系列                 |
| --- | --------- | ------------------------ | ----------------------- | ------------------------ |
| 1   | piscope   | 1µs 采样低成本逻辑分析仪 | `pigpiod` + piscope GUI | F429 ch07/09 波形课      |
...
定位提醒（来自本系列 spec 的 non-goals）：piscope 是低成本档——解码字节够用，量亚微秒时序不
...
> ⚠️ 类比边界：真逻辑分析仪的采样时钟是独立晶振；pigpiod 靠软件循环采样，时间戳有 µs 级抖动（正是下文「低成本档」边界的来源）。

**用途**：把 RPi 的若干 GPIO 当采样探针，1µs 采样率抓数字波形并解码——F429 阶段 2 的 SPI/UART
波形课（[[ch09-rpi-piscope-logic-analyzer|F429 ch09]]）在 LA 到货前的主机位。

**装备与接线**（探 F429 的 SPI，波形源 =
[[ch07-spi-flash-w25q64|F429 ch07]] 的 W25Q64 实验）：
...
[[ch05-request-irq-vs-nvic|ch05]] 按键实验同跑，GPIO4 让给探针时先 rmmod。）
...
- pigpiod 靠 `/dev/mem` 采样——[[ch02-devmem-direct-register|ch02]] 的
...
**用途**：给 F429 自写 I2C 从机（[[ch08-i2c-dual-board-rpi-master|F429 ch08]]）
...
**用途**：抓 F429 以太网实验（[[ch20-ethernet-emac-phy|ch20]] 链路建立、
[[ch21-baremetal-lwip|ch21]] 裸机 lwIP）**上线首包**：ARP/DHCP/ICMP 三件套
...
- pcap 离线分析直接交给
  [[extra-pcapx-pluggable-capture|lwIP 附加篇：pcapx]] 的套路与
  [[ch23-debugging-toolbox|lwIP（二十三）：调试工具箱]]——抓下来的
  文件是三个系列共用的证据格式。
...
[[ch22-esp32-vs-f429-throughput|F429 ch22]] 的全家对照当对端/参照。
...
[[ch12-tcp-reliability-flow-control|lwIP（十二）：TCP 可靠性与流控]]），
链路基线再高，lwIP 侧也到不了。
...
[[ch15-hands-on-measurements|WiFi RF（十五）：动手测量]] 与
[[ch16-rate-negotiation|WiFi RF（十六）：速率协商]] 提供真机核销环境。
...
- **regdomain**：`country_code` 与 `iw reg get` 要一致（CN 的 2.4G 功率/信道限制），不一致时
  ...
  [[wifi-rf-hw|WiFi RF 系列索引]] 第 17 章）在实机上先收
  一笔利息；
...
[[ch5-image-and-flashing|BOX-3 第五章]]讲过它的原理）**插上笔记本/RPi
