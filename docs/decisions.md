...
## KD-14: 表格三联缺陷——wikilink 裸竖线/转义断开/分隔行列数错 — RESOLVED 2026-08-29

Phenomenon: 线上 ch2 的分区表显示为原始 Markdown；全库排查发现三种同源缺陷：
① 表格单元格内 `[[a | b]]` 的竖线未转义，把行劈成多列；② prettier 把残骸
排版成 `\`+空格+`|` 的「断开的转义」，渲染仍碎；③ 分隔行被撑多的格数与
表头不一致，GFM 判整表非法。
Root cause: 写作侧不知道「表格内竖线必须 `\|`」；prettier 对已损坏表格的
再排版掩盖了症状。新三系列 18 行 + 4 张分隔行 + 3 个衍生破损行。
Decision: 三步修复（perl 转义缝合 → 分隔行/行内容手工重排 → 终检扫描器
「全表行列数一致性」归零），`npx quartz build` 后验证 `<table>` 真实渲染。
历史遗留（vpp/ebpf/kernel/rdma/vpn/quic/suricata 等 11 文件约 458 行）
未处理，待用户决定是否批量修。规矩沉淀：**含 `|` 的内容进表格一律 `\|`**。
Impact: 新三系列 14 文件；quartz 本地构建命令注意 `-d` 是内容目录而非输出。

## KD-15: 分隔行误用 `\|`——KD-14 规矩的镜像错误 — RESOLVED 2026-08-29

Phenomenon: WiFi 硬件系列 ch5/ch14 各 10 张表的**分隔行**被写成 `\|`，
GFM 判整表非法；两章作者均声称这是「prettier 合规形态」（ch5 起头、
ch14 效仿），ch17 作者复核 KD-14 时举报。
Root cause: KD-14 沉淀的「含竖线一律 `\|`」被过度泛化到分隔行——分隔
行必须由裸 `|`/`-`/`:`/空格 构成；且既有终检扫描器只匹配合法分隔行，
对含反斜杠的「疑似分隔行」直接跳过，形成检测盲区。
Decision: perl 还原两章分隔行（`\|`→`|`）；扫描器补「疑似转义分隔行」
检测分支；KD-14 规矩修订为两条：**单元格内容内的竖线写 `\|`；分隔行
永远裸竖线**。
Impact: ch5/ch14 共 20 张表修复、全系列表格终检归零；写作任务书模板
补此规则，后续批次不再复现。

## KD-16: 工作区误重置吞掉未跟踪稿件——转录恢复通道 — OPEN 2026-08-29

Phenomenon: WiFi 硬件系列 17 章+索引+spec（均为未跟踪新文件）在工作区
被重置时全部消失；用户自述「不小心中断」。已提交内容无损，并行会话的
2026-08-30 新产物无损。
Root cause: 未跟踪文件不进 git，任何 clean/checkout 类操作对其零保护；
长周期多批次产出的「已验收未提交」窗口期风险被暴露。
Decision: 三通道恢复——①spec/索引主会话上下文直接重写（已完成）；
②章节全文从 agent 会话转录（~/.zcode/cli/agents/<sess>/agent\_\*/output.txt
的 Write tool_use 载荷）程序化提取重建（恢复 agent 执行中，取每文件最后
一次 Write、json 解析、按预期行数核对）；③在跑的 ch15/16 agent 自行
落盘。恢复后重放主会话的后处理（ch5/14 分隔行修复、prettier、KD-14/15
扫描器）。
Impact: 本次恢复；流程教训——**多 agent 长批次写作应分批提交或至少
git stash 备份，不留「已验收未提交」长窗口**；转录目录即事实上的
工作备份。

## KD-17: 扇出任务书的「速查表事实」三处错误被子 agent 实读源码纠正 — RESOLVED 2026-08-30

Phenomenon: F429/RPi 双系列 30 章扇出中，主会话嵌入简报的板卡事实速查表
被三位子 agent 实读核验后抓出三处硬错误：①Bookworm 的 libgpiod 实为
1.6.3（v1 系）非 v2（RPi ch03 作者查 Debian 打包+上游源码纠正）；
②FPSCR 写成内存地址 0xE000EF38——实为 MRS/MSR 专访特殊寄存器，
0xE000EF38 是 FPCAR（ch18 作者对 CMSIS core_cm4.h 纠正）；③EMAC 基址
写成 0x40029000@AHB2——实为 ETH 块 0x40028000@AHB1、DMA 寄存器在
+0x1000（ch20 作者对 RM0090+stm32f429xx.h 纠正）。另有 heap_4 归属
（ucHeap 静态数组 .bss，非链接脚本 .heap 段）等口径纠错。
Root cause: 速查表由主会话凭「验证过的事实+记忆残留」混合写成，未经
二次核验就作为权威基线下发；错误事实混入可信事实后，子 agent 若不执行
实读纪律就会直接进稿。
Decision: 简报纪律双保险固化：①速查表条目注明来源等级（本会话实测/
官方文档/记忆推断），推断级必须标「待核对」；②简报保留既有硬规则
「内核/寄存器行为必须实读真源码引用行号」——本次三处错误全部被该规则
拦截，无一进稿，证明规则有效。速查表本身不废除（启动效率高），但定位
降级为「起点假设」。
Impact: 双系列 30 章交付质量（错误事实零进稿）；后续所有扇出任务书
模板补「来源等级标注」栏。
