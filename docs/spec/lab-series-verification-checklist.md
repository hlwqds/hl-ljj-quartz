...
### ch01 SysTick（✅ 2026-09-01 全章核销，固件 ~/stm32/f429-labs/ch01-systick）

- [x] CALIB 实测值 = 0x4000493E（SKEW=1、TENMS=18750 诡异值——"读前猜不准"教学项成立）
- [x] LED 引脚 = PH10（RGB 红，共阳低电平点亮；官方教程查明+固件驱动；肉眼终验待用户）
- [x] COUNTFLAG 读清语义：固件内双读 c1=0x00010007/c2=0x00000007 实锤（调试器读有回卷混叠的遗留问题就此裁决）
- [x] 节拍漂移：65 秒精测 +40.9 ppm（25M 晶振典型公差内）
- [x] FreeRTOS 固件下 SHPR3 = 0xF0F00000
- [x] 裸机版附加战果：CTRL/LOAD/VAL 活体读与推导全对账（LOAD=0x2BF1F 一字不差）；g_ms 速率 1000.5Hz
...
| #   | 缺口                       | 影响章节    | 核销方式               | 状态                                                                                                                                                                                                                                                  |
| --- | -------------------------- | ----------- | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | W25Q64 挂载控制器/引脚     | ch07/09/22  | 野火教程/原理图/万用表 | ✅ **2026-09-01 官方教程查明：V2 核心板实为 W25Q256(32MB)@SPI5**——SCK=PF7/MISO=PF8/MOSI=PF9/CS=PF6(软件 GPIO)、SPI 模式 3、MSB first、分频 2；JEDEC ID 预期改 EF 40 19；需 4 字节地址模式初始化（ch07 已加勘误）。芯片丝印终验待做                    |
| 2   | KEY1/KEY2、LED 引脚号      | ch01/02/03  | 原理图/丝印            | ✅ **2026-09-01 官方教程查明：KEY1=PA0、KEY2=PC13**（按下=高电平、电路自带接地、NOPULL、**带硬件消抖电容**——ch02 的"低有效+需软件消抖"假设需修正）；**RGB 灯=PH10(R)/PH11(G)/PH12(B) 共阳、低电平点亮**（推挽输出 0=亮）。J73 与 RGB 的关联待板上确认 |
| 3   | PHY 型号/地址/REF_CLK 方案 | ch20-22     | 丝印+MDIO 扫描         | ⬜ 待核（丝印需低头看板）                                                                                                                                                                                                                             |
| 4   | PI11 复用与蜂鸣器驱动电路  | ch03        | 原理图                 | ⬜ 待核（PI11=蜂鸣器已由底板介绍文档证实；驱动电路形态待查）                                                                                                                                                                                          |
| 5   | I2C1 PB6/PB7 是否引出      | ch08        | 排针丝印               | ⬜ 待核                                                                                                                                                                                                                                               |
| 6   | fx2lpo 逻辑分析仪到货      | ch09 对照   | 采购 ~¥50              | ⬜ 在途                                                                                                                                                                                                                                               |
| 7   | 杜邦线/LED/电阻小包        | RPi ch02-04 | 采购 ~¥10              | ✅ 杜邦线已到位（手接 SWD 实测用过）；万用表/LA 待购                                                                                                                                                                                                  |
| 8   | LAN8742 PHY ID2 值         | ch20        | Microchip 手册         | ⬜ 待核                                                                                                                                                                                                                                               |

来源：野火《零死角玩转 STM32-F429》第 7/12 章（cnblogs firege）与
F429 挑战者标准库教程 SPI 章（doc.embedfire.com）。
